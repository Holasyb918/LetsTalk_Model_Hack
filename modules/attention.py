...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.norm_type == "ada_norm":
            norm_hidden_states = self.norm0(hidden_states, timestep)
        elif self.norm_type == "ada_norm_zero":
            norm_hidden_states, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.norm0(
                hidden_states, timestep, class_labels, hidden_dtype=hidden_states.dtype
            )
        elif self.norm_type in ["layer_norm", "layer_norm_i2vgen"]:
            norm_hidden_states = self.norm0(hidden_states)
        elif self.norm_type == "ada_norm_continuous":
            norm_hidden_states = self.norm0(
                hidden_states, added_cond_kwargs["pooled_text_emb"]
            )
        elif self.norm_type == "ada_norm_single":
            # print(self.scale_shift_table[None].shape, timestep.shape); exit(-1)
        ...
        attn_output = rearrange(
            attn_output, "(b h w) f c -> (b f) (h w) c", f=num_frames, h=height, w=width
        )
        ...
        # 1. Audio Cross Attention
        # norm normal Layer Norm
        ...
        hidden_states = torch.cat((hidden_states, reference_image_latents), dim=1)
        if self.norm_type == "ada_norm":
        ...
        hidden_states = attn_output + hidden_states
        seq_len = hidden_states.shape[1] // 2
